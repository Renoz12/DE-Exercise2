...
CUSTOMER_CSV = "/data/Customer"
NETSALE_URL = "/data/Netsale"
...
def get_csv_to_rds(csv_path,rds_name):
    df = pd.DataFrame(r = requests.get(csv_path))

    # เปลี่ยนจาก index ที่เป็น date ให้เป็น column ชื่อ date แทน แล้วเซฟไฟล์ CSV
    #df = df.reset_index().rename(columns={"index": "date"})
    #df.to_csv(rds_name, index=False)
    print(f"Output to {rds_name}")
    print(df)
...
with DAG(
    "get_total_netsale",
    start_date=pen.today('UTC').add(days=-1),
    schedule="@once",
    tags=["exercise2"]
) as dag:

    dag.doc_md = "exercise2"
    ...
    t2 = PythonOperator(
        ...
        python_callable=get_csv_to_rds,
        op_kwargs={
            "csv_path": CUSTOMER_CSV,
            "rds_name": "CUSTOMER",
        },
    )
    t2
